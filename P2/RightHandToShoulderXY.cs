...
{
    public class RightHandToShoulderXY
    {
        ...
        { // Idea: to search for elbow's angle .
            ...
            keyAngle = calcAngleXZ(vShoulderElbow, vElbowWrist); // Calculate key angle between vectors..

            // I need shoulder and elbow in the same height (more or less)
            // and same depth between wrist and elbow! Dont cheat!!
            similarPos = similarY(shoulder, elbow);
        ...
        private double calcAngleXZ(myPoint v1, myPoint v2)
        {   // This functions calculates the angle between two vectors.
            double cosin = (v1.x * v2.x) + (v1.z * v2.z);
            double sum1 = Math.Sqrt((v1.x * v1.x) + (v1.z * v1.z));
            double sum2 = Math.Sqrt((v2.x * v2.x) + (v2.z * v2.z));
        ...
        public List<SkeletonPoint> updateGoals(Skeleton skeleton)
        {
            ...
            elbowHelp.Z = elbowHelp.Z + (float)elbowWristLength;
